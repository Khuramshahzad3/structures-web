    ...
    return (
        <div className="bg-white relative z-50 mx-auto">
            <div className="drop-shadow-2xl mx-auto sm:px-16  lg:flex lg:items-center lg:justify-between  py-0 px-6 ">
                    ...
                    <ul className="lg:flex hidden lg:flex-row xl:text-[1.3vw]  lg:items-center lg:justify-between lg:gap-10 py-5    ">
                        {Menu.map((link, index) => (
                            <li key={index} className="relative group ">
                                <Link to={link.routes} className="flex items-center font-Poppins text-[#002E40] hover:text-[#809320]">
                                    {link.name}
                                    {/* {(index === 1 || index === 2) && (
                ...
                {/* mobile nav */}
                <div className="     ">
                    <ul className={`lg:hidden w-[90vw] xs:w-[70vw] pl-10 flex bg-white rounded-xl flex-col absolute justify-between gap-5 py-10 duration-500 text-[#002E40] font-Poppins  ${open ? "left-0 " : "left-[-100%]"}`}>
                        {Menu.map((link, index) => (
                            <li key={index} className="relative group">
